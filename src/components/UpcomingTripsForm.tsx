...
const labelCls = "block text-foreground/45 text-[0.65rem] tracking-[0.22em] uppercase font-sans mb-2";
const errCls = "text-red-400/65 text-xs font-sans mt-2 leading-snug";
const optionalNote = <span className="normal-case tracking-normal opacity-35"> — optional</span>;
...
function SectionHeader({ n, title }: { n: string; title: string }) {
  return (
    <div className="flex items-center gap-4 mb-8">
      ...
      <span className="text-[#c4813d] text-[0.65rem] tracking-[0.32em] uppercase font-sans shrink-0">{title}</span>
      <div className="h-px flex-1 bg-border" />
    </div>
  );
}
...
const TRIP_OPTIONS = [
  { value: "Guadalupe Mountains — December 9–13, 2026", label: "Guadalupe Mountains", dates: "December 9–13, 2026" },
  { value: "Big Bend — January 13–17, 2027",            label: "Big Bend",             dates: "January 13–17, 2027" },
];

function TripSelect({ value, onChange, error }: { value: string; onChange: (v: string) => void; error?: string }) {
  return (
    <div>
      <div className="grid sm:grid-cols-2 gap-px bg-border">
        {TRIP_OPTIONS.map((trip) => {
          const active = value === trip.value;
          return (
            <button
              key={trip.value}
              type="button"
            ...
            >
              <span className="font-display text-xl text-foreground leading-tight">{trip.label}</span>
              <span className={`text-sm font-sans ${active ? "text-[#c4813d]" : "text-foreground/40"}`}>
                {trip.dates}
              </span>
            </button>
          );
...
function ToggleGroup({ value, options, onChange, error }: {
  value: string;
...
const TIERS = [
  { id: "supporter", label: "Supporter", price: "$1,200", note: "Helps subsidize a community spot" },
  { id: "standard",  label: "Standard",  price: "$800",   note: "Full trip experience" },
  { id: "community", label: "Community", price: "$400",   note: "Limited — need-based" },
];

function TierCards({ value, onChange, error }: { value: string; onChange: (v: string) => void; error?: string }) {
  return (
    <div>
      <div className="grid sm:grid-cols-3 gap-px bg-border">
        {TIERS.map((tier) => {
          ...
          return (
            ...
            >
              <span className="font-display text-xl text-foreground leading-tight">{tier.label}</span>
              <span className={`text-2xl font-sans font-semibold ${active ? "text-[#c4813d]" : "text-foreground/55"}`}>
                {tier.price}
              </span>
              <span className="text-foreground/30 font-sans text-xs leading-relaxed">{tier.note}</span>
            </button>
          );
        })}
      </div>
...
export default function UpcomingTripsForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setServerError(`Submission failed: ${json.error ?? "Unknown error"}${json.hint ? ` — ${json.hint}` : ""}`);
        setLoading(false);
        return;
      }
    } catch {
  ...
  if (submitted) {
    return (
      <div className="border-l-2 border-[#c4813d] pl-8 py-4 max-w-xl">
        <h3 className="font-display text-3xl text-foreground mb-5">Your application has been received.</h3>
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-12">
      ...
      <fieldset className="border-0 p-0 m-0">
        <SectionHeader n="00" title="Which Trip" />
        <div data-has-error={errors.trip_name ? "true" : undefined}>
          <label className={labelCls}>Which trip are you applying for? *</label>
          <TripSelect value={form.trip_name} onChange={(v) => set("trip_name", v)} error={errors.trip_name} />
        </div>
      </fieldset>
        ...
        <div className="flex flex-col gap-6">
          <div className="grid sm:grid-cols-2 gap-6">
            <div data-has-error={errors.full_name ? "true" : undefined}>
              ...
              <input name="full_name" value={form.full_name} onChange={onChange} className={inputCls} placeholder="John Muir" />
              {errors.full_name && <p className={errCls}>{errors.full_name}</p>}
            </div>
            <div data-has-error={errors.age ? "true" : undefined}>
              <label className={labelCls}>Age *</label>
              <input name="age" value={form.age} onChange={onChange} className={inputCls} placeholder="34" />
          ...
          <div data-has-error={errors.city_state ? "true" : undefined}>
            ...
            <input name="city_state" value={form.city_state} onChange={onChange} className={inputCls} placeholder="Denver, CO" />
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            <div data-has-error={errors.email ? "true" : undefined}>
              ...
              <input name="email" type="email" value={form.email} onChange={onChange} className={inputCls} placeholder="john@example.com" />
              {errors.email && <p className={errCls}>{errors.email}</p>}
            </div>
            <div data-has-error={errors.phone ? "true" : undefined}>
              <label className={labelCls}>Phone *</label>
              <input name="phone" type="tel" value={form.phone} onChange={onChange} className={inputCls} placeholder="+1 (555) 000-0000" />
          ...
          <div data-has-error={errors.heard_about_us ? "true" : undefined}>
            ...
            <input name="heard_about_us" value={form.heard_about_us} onChange={onChange} className={inputCls} placeholder="A friend, Instagram, podcast…" />
      ...
      <fieldset className="border-0 p-0 m-0">
        ...
        <div className="flex flex-col gap-6">
          <div data-has-error={errors.fitness_level ? "true" : undefined}>
            ...
            <textarea name="fitness_level" value={form.fitness_level} onChange={onChange} rows={3} className={textareaCls} placeholder="How often you train, what activities, your current shape…" />
          ...
          <div data-has-error={errors.backpacking_experience ? "true" : undefined}>
            ...
            <textarea name="backpacking_experience" value={form.backpacking_experience} onChange={onChange} rows={3} className={textareaCls} placeholder="Trips completed, terrain types, longest multi-day you've done…" />
          ...
          <div data-has-error={errors.mileage_comfortable ? "true" : undefined}>
            <label className={labelCls}>Are you comfortable covering 7–12 miles per day on varied terrain? *</label>
            <ToggleGroup
          ...
          <div>
            ...
            <textarea name="physical_limitations" value={form.physical_limitations} onChange={onChange} rows={2} className={textareaCls} placeholder="Injuries, conditions, medications — anything we should be aware of…" />
          </div>
        </div>
      </fieldset>
      ...
      <fieldset className="border-0 p-0 m-0">
        ...
        <div className="flex flex-col gap-6">
          <div data-has-error={errors.what_drew_you ? "true" : undefined}>
            ...
            <textarea name="what_drew_you" value={form.what_drew_you} onChange={onChange} rows={4} className={textareaCls} placeholder="What called you here…" />
          ...
          <div data-has-error={errors.hoping_to_gain ? "true" : undefined}>
            ...
            <textarea name="hoping_to_gain" value={form.hoping_to_gain} onChange={onChange} rows={4} className={textareaCls} placeholder="What would make this week meaningful for you…" />
            {errors.hoping_to_gain && <p className={errCls}>{errors.hoping_to_gain}</p>}
          </div>

          <div data-has-error={errors.life_connection ? "true" : undefined}>
            <label className={labelCls}>Is there something in your life right now connected to why you said yes? *</label>
            <textarea name="life_connection" value={form.life_connection} onChange={onChange} rows={4} className={textareaCls} placeholder="A season, a transition, something you're working through or toward…" />
      ...
      <fieldset className="border-0 p-0 m-0">
        ...
        <div className="flex flex-col gap-6">
          <div data-has-error={errors.how_others_describe ? "true" : undefined}>
            ...
            <textarea name="how_others_describe" value={form.how_others_describe} onChange={onChange} rows={3} className={textareaCls} placeholder="Be honest — the good and the complicated…" />
          ...
          <div data-has-error={errors.brotherhood_meaning ? "true" : undefined}>
            <label className={labelCls}>What does brotherhood mean to you — and do you have it in your life? *</label>
            <textarea name="brotherhood_meaning" value={form.brotherhood_meaning} onChange={onChange} rows={4} className={textareaCls} placeholder="What it looks like, where you find it, where it falls short…" />
          ...
          <div data-has-error={errors.rite_of_passage ? "true" : undefined}>
            ...
            <textarea name="rite_of_passage" value={form.rite_of_passage} onChange={onChange} rows={4} className={textareaCls} placeholder="A moment, an experience, a threshold you crossed…" />
          ...
          <div data-has-error={errors.what_you_bring ? "true" : undefined}>
            <label className={labelCls}>What do you bring to a group — and what do you struggle with? *</label>
            <textarea name="what_you_bring" value={form.what_you_bring} onChange={onChange} rows={4} className={textareaCls} placeholder="Your gifts and your edges…" />
      ...
      <fieldset className="border-0 p-0 m-0">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div data-has-error={errors.transportation ? "true" : undefined}>
            ...
            <textarea name="transportation" value={form.transportation} onChange={onChange} rows={2} className={textareaCls} placeholder="Flying in, driving from home, need to coordinate from a hub city…" />
          ...
          <div>
            ...
            <input name="dietary_restrictions" value={form.dietary_restrictions} onChange={onChange} className={inputCls} placeholder="Allergies, preferences, anything to note…" />
          </div>
          ...
          <div data-has-error={errors.investment_tier ? "true" : undefined}>
            ...
            <p className="text-foreground/28 font-sans text-xs mb-4 leading-relaxed">
              Choose the tier that fits your situation. Community spots are limited and reviewed on a case-by-case basis.
            </p>
            <TierCards value={form.investment_tier} onChange={(v) => set("investment_tier", v)} error={errors.investment_tier} />
          </div>
        </div>
      </fieldset>
      ...
      <fieldset className="border-0 p-0 m-0">
        ...
        <div>
          ...
          <textarea name="anything_else" value={form.anything_else} onChange={onChange} rows={4} className={textareaCls} placeholder="Open floor — say whatever feels important…" />
        </div>
      </fieldset>
